...
def rbgbout_hyb( infiles, outfiles, stage=None, cp=None):
    ...
    strength = cp.getfloat(stage, 'strength')
    ...
    for i, infile in enumerate(infiles):
        ...
        Ifilt = read_image( infile, channels = select_indexes)      
        # Ifilt=np.zeros_like(I)
        I_rgb= np.zeros((3,Ifilt.shape[1], Ifilt.shape[2]), dtype=np.double)
        ...
        Irgb_transposed = np.transpose( Irgb, axes=(1,2,0) )

        logging.debug(f'done processing {base}.{ext} ')
        logging.info(f'writing to {outfile}')
        write_image( outfile, Irgb_transposed , photometric='rgb' )
